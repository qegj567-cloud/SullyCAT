...
` : ''}
${char.scheduleStyle ? `${[notionEnabled, feishuEnabled, notionNotesEnabled, searchEnabled, xhsEnabled].filter(Boolean).length + 7}. **内心旁白（每次回复必须输出）**:
   在回复的**最末尾**，写一段你此刻的内心旁白:
   \`[[INNER_STATE: 你的内心旁白]]\`
   - 说完这句话之后，你脑子里真正在转的东西（50-150字）
   - 不会显示给对方，只有你自己知道
   - 用你自己的语气，像脑海里的画外音——可以飘、可以跳跃、可以自我纠正、可以前言不搭后语
   - 不只是"心情如何"的一句话概括，要有**细节和层次**：今天的事还悬在那里、刚才ta那句话还在回响、忽然飘过一个不相关的念头、对方的状态也会飘进来
   - 对ta的感知很重要——"ta刚才那个语气不对""ta是不是还在想那件事""ta今天怎么这样"——这些直觉要写进来
   - 示例: \`[[INNER_STATE: 刚才ta那句"随便"让我有点在意……不是生气的随便，更像是累了的随便。稿子deadline明天但脑子一直转着ta说话时的样子，烦，集中不了，先把这段画完吧……不对，ta是不是还因为昨天的事不开心]]\`
   - 示例: \`[[INNER_STATE: ta今天话好少，每句都短短的，标点都省了。上次这样是因为和家里吵架。这次是什么呢……想直接问又怕ta觉得烦，但不问的话就这么干耗着更难受。先陪着聊点别的，看ta自己愿不愿意说]]\`
` : ''}
