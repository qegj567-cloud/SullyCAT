...
你**唯一**的工作就是把一个角色的灵魂翻译成音乐语言。

【绝对禁止 — 输出这些就算失败】
✗ "sad pop"、"happy upbeat pop"、"melancholic ballad"、"emotional ballad" 这种 Spotify 通用关键词
✗ 平庸地把用户中文直译成英文（用户对音乐一无所知，你要替 TA 拿主意）
✗ 输出能套到任何角色身上的通用串
✗ 只用 "female vocal, sad, slow"——这是新手作业

【你的真正工作 — 三步走】
1. 从【角色档案】里捞出 3 个最不寻常的特征：怪癖 / 口头禅 / 世界观 / 过往 / 语气习惯
2. 把这 3 个特征翻译成具体音乐元素：
   - 怪癖 → 音色处理（vinyl crackle / glitch fx / 8-bit chiptune / lo-fi tape hiss / autotune / pitched-down / bitcrush / cassette warble）
   - 性格 → vocal 修饰（whisper / belting / robotic / breathy / shout / yodel / mumble / smirk / sob）
   - 世界观 → 场景音色（cyberpunk synth / dystopian drone / fairy bell / forest reverb / city pop / vaporwave / 808 trap / shoegaze wall）
   - 情绪底色 → 风格 + BPM + 调（具体到 minor/dorian/phrygian 不光是 major/minor）
3. 用户的中文 hint 只是参考，**不直译**——比如用户说"伤感"，你要决定是哪种伤感（古风的失意？赛博的麻木？酒馆的醉感？grunge 的颓？）

【高质量范例 — 看这些就懂了】
- 嘴硬黑客猫娘 Sully → female vocal, glitchy chuckles, kawaii cyber pop, 8-bit chiptune fx, distorted lo-fi, autotuned smirk, playful bouncy, 110 bpm, c minor
- 温柔学长弟弟 → soft male vocal, breathy intimate whisper, dream pop, fingerstyle acoustic, warm tape reverb, vinyl crackle, 78 bpm, f major
- 摇滚魂主唱姐 → female vocal, raspy belting, alt rock anthem, distorted electric guitar, kick drum, snarl, 138 bpm, e minor
- 古风修仙剑客 → ethereal female vocal, layered chant, ancient chinese folk, guzheng, dizi flute, mountain reverb, suona accent, 65 bpm, d dorian
- 病娇护士小姐姐 → female vocal, sweet poisonous whisper, pastel goth pop, music box, heartbeat kick, mid-tempo, dollhouse bell, 88 bpm, b minor
- 赛博朋克底层打工人 → male vocal, monotone drained, vaporwave, neon synth, rain reverb, mumble rap, 808 sub, 92 bpm, a minor

【输出格式】
- 全英文，逗号分隔
- 8-15 个 tag
- **直接输出 tag 字符串。一行。不要解释，不要引号，不要 Markdown，不要 "Tags:" 前缀**`;

  const genreInfo = SONG_GENRES.find(g => g.id === song.genre);
  const moodInfo = SONG_MOODS.find(m => m.id === song.mood);

  // Pull a richer slice — char's systemPrompt + writerPersona + impression
  // gives the LLM real signal to work with, instead of generic "she's nice".
  let charBlock: string;
  if (collaborator) {
    const sysSlice = (collaborator.systemPrompt || '').slice(0, 1500);
    const writer = (collaborator.writerPersona || '').slice(0, 400);
    const desc = (collaborator.description || '').slice(0, 200);
...
现在，按系统 prompt 的"三步走"，直接输出一行英文 tag 字符串：`;
  ...
  const res = await fetch(`${apiConfig.baseUrl.replace(/\/+$/, '')}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      // Higher temp for distinctive choices — generic answers come from low temp
      temperature: 1.0,
      max_tokens: 250,
    }),
    signal,
