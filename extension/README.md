...
1. 进过一次 `h5.waimai.meituan.com`（饿了么/盒马同理）登录 + 选地址，让浏览器记住 cookies
2. 打开 SullyOS → 饭友 App
3. 顶部应当看到"扩展已就绪"小绿点 + 一行"📍 已记住地址：xxx"。没看到的话刷新一下 SullyOS 页面
4. 跟 char 聊"想吃啥"——它选好后会调 `execute_in_browser` 工具
5. 浏览器自动开新标签页 → 加好购物车 → 跳到结算页 → **你点支付**

### 定位记忆（0.1.1 起）

每次扫店 / 看菜单时，platform 脚本会把当前 `lat/lng` 回传给扩展，存进 `chrome.storage.local`。
后续新开 tab 时优先用这个值拼到 URL `?lat=&lng=`，**不会再每次都弹"授权定位"**。

控制入口有两个：

- **扩展图标 popup**：点 chrome 工具栏上的扩展图标，可以看当前记住的定位、清除、或一键打开美团 H5 重选
- **饭友 App 内的状态条**：第二行"📍 已记住地址"右侧有"换地址 / 清除"按钮

清掉后下次让 char 搜店时会重新捕获——不是必须手动同步。

### in-page 搜索 fallback（0.1.1 起）

如果美团 H5 的 deep-link search 路由没渲染出店（route 偶尔吃不到 keyword），platform 脚本会
自动找页面里的搜索输入框，用 React-friendly 的 setter + Enter 事件链主动驱动一次搜索。
这是一道兜底——**不是绝对靠谱**，平台改 selector 一样会失效。
...
## 调试 React props（如果 storeId 还是抓不到）

Meituan H5 是 React 应用，店铺/菜品 ID 都存在 React 组件的 props 里，
DOM 上没有任何 ID 字段。扩展用 `__reactProps$` / `__reactFiber$` 内部 key 去抠
props 然后递归找 shop-like 字段。

如果还是抠不到（char 一直拿 synth ID），打开扩展开的 meituan tab，F12
→ Console，跑这段诊断脚本：

```js
const card = document.querySelector('.poilist-item');
if (!card) { console.warn('找不到 .poilist-item，selector 全过期了'); }
else {
  ...
  console.log('fiberKey:', fiberKey, 'propsKey:', propsKey);
  if (propsKey) {
    console.log('props =', card[propsKey]);
  }
  if (fiberKey) {
    let f = card[fiberKey];
    for (let i = 0; i < 8 && f; i++, f = f.return) {
      console.log(`fiber[${i}].memoizedProps =`, f.memoizedProps);
    }
  }
}
```

把 console 里看到的对象结构截图给开发者，我能据此调字段名。
