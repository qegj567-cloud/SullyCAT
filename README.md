...
> 「叮叮叮！检测到要被顺走的不只是源码……资产保护程序启动。」

协议条款写的是"软件"，但这项目里**不是代码的部分也是我的著作权**——图标、banner、Sully 立绘、角色人设、内置角色的设定书和台词、UI 文案、攻略本里的剧情、世界书里的一切——按《著作权法》单独受保护，和代码分开算账。

**不是想卡死，就划一下边界**：

- ✅ **整个项目一起拿去玩/魔改/fork**：图、角色、文案跟着走，没问题，本来就该这样。
- ✅ **改完发出去给别人玩**：带着原始素材 OK，署名别删。
- ❌ **把 `pics/`、`icons/`、`assets/` 里的图扒出来发素材站 / 网盘 / 小红书当"免费素材包"**——不行。
- ❌ **把 Sully 的人设 / 台词 / 内置角色的设定书搬进你自己的 AI 项目当"免费角色包"派发**——不行。
- ❌ **商用一律不行**，和代码同等待遇。

一句话：**整个拿去造随便造，单独拆零件派发就算了。**
